...
(function($) {
    $.fn.jPaginator = function(o) {
        ...
        return this.each(function() {
            ...
            $(this).bind('next', function(event, o) {
                //console.log(o);
                s.selectedPage = ((o.current_page - 1)  >= s.nbPages) ? s.nbPages : o.current_page;

                c.listenSlider = false;
                $(this).find(".paginator_slider").slider("option", "value", Math.round((s.selectedPage * 100)/s.nbPages));
                c.listenSlider = true;

                if (s.onPageClicked && c.custom) {
                    c.custom = false;
                    setTimeout(function() {
                        //console.log('s.selectedPage : ' + s.selectedPage);
            ...
            $(this).bind('prev', function(event, o) {
                //console.log(o);
                s.selectedPage = (o.current_page <= 0) ? 1 : o.current_page;
            ...
            function handleSliderChange(e, ui) {
                ...
                if (!c.isMoving) {
                    if(c.custom) trackSlidetoPage(e, ui);
                    moveToPc(ui.value);
                }
            };

            function trackSlidetoPage(e, ui) {
                if (!c.listenSlider)
                    return;

                if (!c.isMoving) {
                    s.selectedPage = Math.round((ui.value * s.nbPages) / 100);
                    s.selectedPage = (s.selectedPage == 0) ? 1 : s.selectedPage;
                    updateNum(s.selectedPage);
                    //console.log('current page is : ' + s.selectedPage);

                    setTimeout(function() {
                        if (s.onPageClicked && e.type === "slidechange")
                            s.onPageClicked.call(this, $this, s.selectedPage);
                    }, 3*1000);
                }
            ...
            function init() {
                ...
                $this.find(".paginator_slider").width('80%');
