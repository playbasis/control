...
class Pipedrive extends REST_Controller{
	...
	public function send_post(){
		...
		$required = $this->input->checkParam(array('company', 'person', 'email', 'users_size', 'budget'));
		if($required)
	...
	private function auth(){
		...
		$data = array ('email' => 'rob@playbasis.com', 'password' => 'r0b3rt21play');
		$opts = array (
			'http' => array (
				'method' => 'POST',
	...
	private function createPerson($token, $name, $orgId, $email){
		...
		$data = array ('name' => $name, 'org_id' => $orgId, 'email' => $email);
		$opts = array (
			'http' => array (
					'method' => 'POST',
					'header'=> "Content-type: application/json",
