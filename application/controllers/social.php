...
class Social extends REST_Controller
{
	...
	public function test_get()
	{
		...
		$pb_player_id = $this->social_model->getPBPlayerIdFromFacebookId($facebook_id, $token['client_id'], $token['site_id']);
		print_r($pb_player_id);
		echo '<br>getPBPlayerIdFromFacebookId<br>';
		$pb_player_id = $this->social_model->getPBPlayerIdFromTwitterId($twitter_id, $token['client_id'], $token['site_id']);
		print_r($pb_player_id);
		echo '<br>getPBPlayerIdFromInstagramId<br>';
		$pb_player_id = $this->social_model->getPBPlayerIdFromInstagramId($instagram_id, $token['client_id'], $token['site_id']);
		print_r($pb_player_id);
		echo '</pre>';
	}
}
