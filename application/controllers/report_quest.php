...
class Report_quest extends MY_Controller
{
    ...
    private function getDefaultQuestReportDateStart()
    {
        return date("Y-m-d H:i:s", strtotime(date("Y-m-d", strtotime("-30 days"))));
    }

    private function getDefaultQuestReportDateEnd()
    {
        return date("Y-m-d H:i:s", strtotime(date("Y-m-d")) + 86399);
    }

    private function getQuestReportDateFilter($value, $fallback, $end_of_day = false)
    {
        if (!is_string($value)) {
            return $fallback;
        }
    ...
    public function getQuestsList($offset, $url)
    {
        ...
        if ($this->input->get('date_start')) {
            $filter_date_start = $this->getQuestReportDateFilter(
                $this->input->get('date_start'),
                $this->getDefaultQuestReportDateStart()
            );
            $parameter_url .= "&date_start=" . $filter_date_start;
        } else {
            $filter_date_start = $this->getDefaultQuestReportDateStart();
        }

        if ($this->input->get('date_expire')) {
            $filter_date_end = $this->getQuestReportDateFilter(
                $this->input->get('date_expire'),
                $this->getDefaultQuestReportDateEnd(),
                true
            );
            $parameter_url .= "&date_expire=" . $filter_date_end;
        } else {
            $filter_date_end = $this->getDefaultQuestReportDateEnd();
        }
