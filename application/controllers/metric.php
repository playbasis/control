		...
		if ($rewardType == 'badge'){
			foreach($badges['response']['badges'] as $badge){
				...
				switch($rewardCompareType){
					case 'percentage':
						$comparedResult[$n1][$badge_id] = $n1ResultRewardValue .'_'.($this->calcDiffPercent($n1ResultRewardValue, $n2ResultRewardValue).'%');
						$comparedResult[$n2][$badge_id] = $n2ResultRewardValue .'_'.($this->calcDiffPercent($n2ResultRewardValue, $n3ResultRewardValue).'%');
						break;
					case 'gross':
