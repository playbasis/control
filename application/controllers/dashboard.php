...
//        $this->data['daily_events'] = $this->Statistic_model->getDailyActionmeaturement($data);
        ...
        if ($client_id) {
            $players = $this->Statistic_model->getLeaderboardByExp($data['limit'], $data['client_id'],
                $data['site_id']);
            if ($players) {
                foreach ($players as $player) {
                    $this->data['leaderboards'][] = array(
                        ...
                        'email' => $player['email'],
                        'level' => $player['level'],
                        'point' => $this->get_player_point_cache($client_id, $site_id, $player['_id'],
                            $cached_player_point),
                        'date_added' => date($this->lang->line('date_format_short'),
                            strtotime($this->datetimeMongotoReadable($player['date_added']))),
                        'last_active' => date($this->lang->line('date_format_short'),
                            strtotime($this->datetimeMongotoReadable($player['date_modified'])))
                    );
                }
            }
        }
        ...
        if ($this->User_model->getUserGroupId() != $this->User_model->getAdminGroupID()) {
            ...
            if ($user_plan['_id'] != FREE_PLAN || $this->User_model->getMobile()) {
                if ($this->User_model->getSiteId()) {
                    $features = $this->Feature_model->getFeatureBySiteId($this->User_model->getClientId(),
                        $this->User_model->getSiteId());
...
//        $this->data['daily_events'] = $this->Statistic_model->getDailyActionmeaturement($data);
        ...
        if (is_array($features)) {
            foreach ($features as $feature) {
                if (array_key_exists('link',
                        $feature) && $feature['link'] == '/' && $this->User_model->hasPermission('access',
                        $feature['link'])
                ) {
                    return true;
                }
    ...
    private function validateAccess()
    {
        ...
        if ($this->User_model->hasPermission('access',
                '/') && $this->Feature_model->getFeatureExistByClientId($client_id, '/')
        ) {
            return true;
        } else {
            return false;
