...
class Badge extends REST_Controller{
	...
	public function index_post($badgeId=0){
		...
		if($badgeId){
			...
			$badge['badge'] = $this->badge_model->getBadge(array_merge($validToken,array('badge_id'=>$badgeId)));
	...
	public function getCollection_post($collectionId  = 0){
		...
		if($collectionId){
			...
			$collection['collection'] = $this->badge_model->getCollection(array_merge($validToken,array('collection_id'=>$collectionId)));
