...
class Report extends CI_Controller
{
    ...
    public function generate($ref = null)
    {
	    ...
	    foreach ($sites as $site) {
		    ...
		    if (!$allowed_site_ids[(string)$site_id]) continue;
	...
	private function get_stat($prefix, $conf, $curr, $prev) {
		...
		$params[$prefix.'BEST_DAY'] = ($best != null ? '('.date(REPORT_DATE_FORMAT, strtotime($curr[$best]['_id'])).')' : '');
	...
	private function master_finalize($conf, &$master) {
		$master = array_merge(
			$master,
			$this->get_basic_stat('GLOBAL_TOTAL_USER_', $conf, $master['GLOBAL_TOTAL_USER_TOTAL_NUM'], $master['GLOBAL_TOTAL_USER_TOTAL_PREV_NUM']),
			$this->get_basic_stat('GLOBAL_NEW_USER_', $conf, $master['GLOBAL_NEW_USER_TOTAL_NUM'], $master['GLOBAL_NEW_USER_TOTAL_PREV_NUM']),
			$this->get_basic_stat('GLOBAL_DAU_', $conf, $master['GLOBAL_DAU_AVERAGE_NUM'], $master['GLOBAL_DAU_AVERAGE_PREV_NUM']),
			$this->get_basic_stat('GLOBAL_MAU_', $conf, $master['GLOBAL_MAU_AVERAGE_NUM'], $master['GLOBAL_MAU_AVERAGE_PREV_NUM'])
		);
	}
}
