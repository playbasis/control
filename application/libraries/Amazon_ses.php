...
class Amazon_ses {
     ...
     * @return 	VerificationToken => A TXT record that must be placed in the DNS settings for the domain, in order to complete domain verification.
     * @author 	Ben Hartard
     */
    public function verify_domain($domain)
    {
        ...
        if($result){
            ...
            return isset($result['VerifyDomainIdentityResult']['VerificationToken']) ? $result['VerifyDomainIdentityResult']['VerificationToken'] : "";
        }else{
            return false;
        }
    }
