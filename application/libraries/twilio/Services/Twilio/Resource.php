...
abstract class Services_Twilio_Resource {
    ...
    public static function decamelize($word)
    {
        return preg_replace_callback(
            '/(^|[a-z])([A-Z])/',
            array(__CLASS__, '_decamelizeCallback'),
            $word
        );
    }
    ...
    public static function camelize($word) {
        return preg_replace_callback('/(^|_)([a-z])/',
            array(__CLASS__, '_camelizeCallback'),
            $word);
    }

    public static function _decamelizeCallback($matches)
    {
        return strtolower(strlen($matches[1]) ? $matches[1] . '_' . $matches[2] : $matches[2]);
    }

    public static function _camelizeCallback($matches)
    {
