...
<script type="text/javascript">
    ...
    var chHost = '<?php echo  preg_replace('/(http[s]?:\/\/)?([w]{3}\.)?/', '', $domain["domain_name"]); ?>';
    ...
    socket.on('message', function(data){
        ...
        notification += '<img class="noti-stream-item-portrait" alt="avatar" src=" ' + data.actor.image.url + '" width="45" height="45"><div class="noti-stream-item-name"><h4> ' + data.actor.displayName + ' </h4><span class="noti-action-act"> ' + data.object.message + ' </span><div class="noti-stream-item-time-stamp" title="'+(Date.parse(data.published).getTime()-(tz*1000))+'"> just now </div></div>';
