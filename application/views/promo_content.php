<div id="content" class="span10">
    <div class="box">
        <div class="heading">
            ...
            <div class="buttons">
                <button class="btn btn-info" onclick="location =  baseUrlPath+'promo_content/insert'" type="button"><?php echo $this->lang->line('button_insert'); ?></button>
                        ...
                        <?php foreach($promo_contents as $promo_content){?>
                            <tr>
                                <td style="text-align: center;"><?php if (isset($promo_content['selected'])) { ?>
                                        <input type="checkbox" name="selected[]" value="<?php echo $promo_content['_id']; ?>" checked="checked" />
                                    <?php } else { ?>
                                        <input type="checkbox" name="selected[]" value="<?php echo $promo_content['_id']; ?>" />
                                    <?php } ?></td>
                                <td class="right"><?php echo $promo_content['name']; ?> <?php if (!empty($promo_content['error'])) { ?><span class="red"><a herf="javascript:void(0)" class="error-icon" title="<?php echo $promo_content['error']; ?>" data-toggle="tooltip"><i class="icon-warning-sign" ></i></a></span><?php } ?></td>
