...
//            $this->email->bcc('cscteam@playbasis.com');
	...
	public function monthy_active_user($data, $from=null, $to=null) {
		$this->set_site_mongodb($data['site_id']);
		$map = new MongoCode("function() { this.date_added.setTime(this.date_added.getTime()-(-7*60*60*1000)); emit(this.date_added.getFullYear()+'-'+('0'+(this.date_added.getMonth()+1)).slice(-2), this.pb_player_id.valueOf()); }");
	...
	public function daily_active_user($data, $from=null, $to=null) {
		$this->set_site_mongodb($data['site_id']);
		$map = new MongoCode("function() { this.date_added.setTime(this.date_added.getTime()-(-7*60*60*1000)); emit(this.date_added.getFullYear()+'-'+('0'+(this.date_added.getMonth()+1)).slice(-2)+'-'+('0'+this.date_added.getDate()).slice(-2), this.pb_player_id.valueOf()); }");
		$reduce = new MongoCode("function(key, values) { return {'pb_player_id': values}; }");
		$query = array('client_id' => $data['client_id'], 'site_id' => $data['site_id']);
		if ($from || $to) $query['date_added'] = array();
		if ($from) $query['date_added']['$gte'] = $this->new_mongo_date($from);
	...
	private function active_user_per_day($data, $ndays, $from=null, $to=null) {
		...
		$map = new MongoCode("function() {
			...
			for (var i = 0; i < ".$ndays."; i++) {
				...
				$check_to
				emit(tmp.getFullYear()+'-'+('0'+(tmp.getMonth()+1)).slice(-2)+'-'+('0'+tmp.getDate()).slice(-2), this.pb_player_id.valueOf());
			}
		}");
		$reduce = new MongoCode("function(key, values) { return {'pb_player_id': values}; }");
		$query = array('client_id' => $data['client_id'], 'site_id' => $data['site_id']);
	...
	private function active_user_per_week($data, $ndays, $from=null, $to=null) {
		...
		$map = new MongoCode("function() {
			...
			for (var i = 0; i < ".$ndays."; i++) {
				...
				if (week > 4) week = 4;
				d = (week-1)*7+1;
				emit(tmp.getFullYear()+'-'+('0'+(tmp.getMonth()+1)).slice(-2)+'-'+('0'+d).slice(-2), this.pb_player_id.valueOf());
			}
		}");
		$reduce = new MongoCode("function(key, values) { return {'pb_player_id': values}; }");
		$query = array('client_id' => $data['client_id'], 'site_id' => $data['site_id']);
	...
	private function active_user_per_month($data, $ndays, $from=null, $to=null) {
		...
		$map = new MongoCode("function() {
			...
			for (var i = 0; i < ".$ndays."; i++) {
				tmp.setTime(this.date_added.getTime()+i*86400000);
				$check_to
				emit(tmp.getFullYear()+'-'+('0'+(tmp.getMonth()+1)).slice(-2), this.pb_player_id.valueOf());
			}
		}");
		$reduce = new MongoCode("function(key, values) { return {'pb_player_id': values}; }");
		$query = array('client_id' => $data['client_id'], 'site_id' => $data['site_id']);
