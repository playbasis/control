...
class Report_player_model extends MY_Model
{
    public function getTotalPlayers($data)
    {
        ...
        $this->mongo_db->where_ne('added_by_script', true);
    ...
    public function getTotalPlayersReferral($data)
    {
        $this->mongo_db->where('client_id', new MongoID($data['client_id']));
        $this->mongo_db->where('site_id', new MongoID($data['site_id']));
        ...
        if (isset($data['username']) && $data['username'] != '') {
            $this->mongo_db->where('$or' , array(array('action_name' => 'invited'), array('action_name' => 'invite')));
            $this->mongo_db->where('cl_player_id', $data['username']);
        } else {
    ...
    public function getReportPlayersReferral($data)
    {
        $this->mongo_db->where('client_id', new MongoID($data['client_id']));
        $this->mongo_db->where('site_id', new MongoID($data['site_id']));
