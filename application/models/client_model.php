...
//			$logData['input'] = serialize(array_merge($logData['input'], $jigsawOptionData));
    ...
    public function listAllActiveClients($refDate=null, $site_id=0) {
        $this->set_site_mongodb($site_id);
        $this->mongo_db->select(array('_id', 'first_name', 'last_name', 'email', 'mobile', 'company', 'date_added', 'paying_ever'));
        $this->mongo_db->where(array('status' => true, 'deleted' => false));
        if ($refDate) $this->mongo_db->where_lt('date_added', new MongoDate($refDate));
    ...
    public function findBySiteId($site_id) {
        $this->set_site_mongodb($site_id);
        $this->mongo_db->select(array('client_id', 'domain_name','site_name'));
